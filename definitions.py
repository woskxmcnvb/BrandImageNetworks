...
"""
реализация двунаправленной связи - correlation
для такой связи указывается только одно ребро model_spec.Edge с типом EDGE_TYPE_CORR
а в каждой вершине model_spec.Node указыватется что есть и входящее и исходящее ребро на другую вершину связи

обходчики графа используют model_spec.Node - получают от них списки вершин, т.о. могут ходить и туда и обратно 
построитель картинки PlotGraph получает pgv.AGraph построенный из списка ребер с типами, таким образом ребро не дублируется 
